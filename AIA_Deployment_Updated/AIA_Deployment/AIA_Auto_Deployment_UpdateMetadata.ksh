...
doScriptLog() {
  ...
  cp ${untarfolder}/data/UpdateMetaData_FULL.xml /u01/app/oracle/soa/installation/weblogic12.1.3/soa/aiafp/Install/config/
  #EXIT_CODE=$?
  return ${EXIT_CODE}
}
...
doDeployLog | tee /u01/app/aialogs/DeployLog.${DATE}
if [ ${EXIT_CODE} -ne 0 ]; then
...
cd /u01/app/oracle/soa/installation/weblogic12.1.3/soa/aiafp/Install/config/

ant -f UpdateMetaData_FULL.xml -logfile /u01/app/aialogs/UpdateMetaData.log_${DATE}
EXIT_CODE=$?
if [ ${EXIT_CODE} -ne 0 ]; then
